...
собственного суффикса) и записывается в `apps/server/package.json` ДО сборки:
`ServerEnvironment` вшивает её в бандл, а десктоп/веб сверяет строки на точное
равенство — любой суффикс даёт вечное предупреждение «Version drift», а если
не штамповать вовсе, сервер представляется версией из upstream (`0.0.31`).
